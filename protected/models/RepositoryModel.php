 ...
 */
class RepositoryModel extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('id', 'required'),
			array('user_id, attitude', 'numerical', 'integerOnly'=>true),
			array('id', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, attitude', 'safe', 'on'=>'search'),
		);
	}
	...
	public static function getRepositories($repository_name){
		try {
			$url = RepositoryModel::URL.'?q='.$repository_name;
			$c = curl_init();
			if (!$c){
